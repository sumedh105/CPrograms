...
static void swapNodesWoSwappingData(struct node **head, int kOne, int kTwo)
{
	struct node *temp = NULL;
	struct node *keyOne = NULL;
	struct node *keyTwo = NULL;
	struct node *posk1 = NULL;
	struct node *posk2 = NULL;

	if (*head == NULL)
	{
		printf("\nThe list is empty, cannot swap anything\n");
	}
	...
	else
	{
		...
		while (keyOne->data != kOne)
		{
			posk1 = keyOne;
			keyOne = keyOne->next;
		}

		while (keyTwo->data != kTwo)
		{
			posk2 = keyTwo;
			keyTwo = keyTwo->next;
		}
...
int main()
{
	...
	while (1)
	{
		...
		printf("\nPress 11 to swap nodes without swapping data in the linked list\n");
		...
		switch(choice)
		{
			...
			case 11:
			{
				...
				printf("\nEnter the two keys which needs to be swapped\n");
				scanf("%d %d", &keyOne, &keyTwo);

				swapNodesWoSwappingData(&head, keyOne, keyTwo);
				break;
			}
