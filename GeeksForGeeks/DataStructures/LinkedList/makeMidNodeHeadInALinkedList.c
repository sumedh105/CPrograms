...
static void makeMidNodeHead(struct node **head)
{
	struct node *temp = NULL;
	struct node *slowPtr = NULL;
	struct node *fastPointer = NULL;
	struct node *posPtr = NULL;

	if (*head == NULL)
	{
		printf("\nThe list is empty, nothing can be done\n");
	}
...
int main()
{
	...
	while (1)
	{
		...
		printf("\nPress 11 to make a middle node head of the linked list\n");
