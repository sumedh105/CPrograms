...
static void reverseALinkedList(struct node **head)
{
	struct node *temp = NULL;
	struct node *prev = NULL;
	struct node *finNode = NULL;

	if (*head == NULL)
	{
		printf("\nLinked list cannot be reversed, it is empty\n");
	}
	else
	{
		temp = *head;
		...
		finNode = temp; //finNode will be the new head.
...
int main()
{
	...
	while (1)
	{
		...
		printf("\nPress 11 to reverse a linked list\n");
