...
static void insertAfterNtNodeFromEnd(struct node **head, int data, int n)
{
	...
	if (*head == NULL)
	{
		printf("\nThe list is empty, a new node cannot be inserted\n");
	}
...
int main()
{
	...
	while (1)
	{
		...
		printf("\nPress 11 to insert after nth node from the end\n");
		...
		switch(choice)
		{
			...
			case 11:
			{
				...
				printf("\nEnter the data and position after which the node must be inserted\n");
				scanf("%d%d", &data, &n);

				insertAfterNtNodeFromEnd(&head, data, n);
				break;
			}
