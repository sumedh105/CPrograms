...
static void reverseInGroup(struct node **head, int size)
{
	struct node *temp = NULL;
	struct node *cur = NULL;
	struct node *prev = NULL;
	struct node *prevCur = NULL;
	...
	while (1)
	{
		...
		switch(choice)
		{
			...
			case 11:
			{
				...
				printf("\nEnter the size k in which the list be reversed in a group\n");
				scanf("%d", &k);
