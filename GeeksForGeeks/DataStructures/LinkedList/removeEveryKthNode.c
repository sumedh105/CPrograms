...
static void removeKthNode(struct node **head, int k)
{
	...
	if (*head == NULL)
	{
		printf("\nNothing can be done, the list is empty\n");
	}
...
int main()
{
	...
	while (1)
	{
		...
		printf("\nPress 11 to remove every kth node from a linked list\n");
		...
		switch(choice)
		{
			...
			case 11:
			{
				...
				printf("\nEnter the kth node to be removed\n");
				scanf("%d", &k);
