...
static int find_height(struct tree *root_node);
static int max(int left_height, int right_height);
static void print_level_order(struct tree *root_node, int height);
static int print_data(struct tree *root_node, int level);
...
int main()
{
	...
	while (1)
	{
		...
		printf("\nEnter 6 to find the height of a tree\n");
		printf("\nEnter 7 to display the tree in level order traversal\n");
		printf("\nEnter the choice\n");
		scanf("%d", &choice);
		...
		switch(choice)
		{
			...
			case 5:
				...
				break;
			case 6:
				height = find_height(root_node);
				printf("\nThe height of a tree is: %d\n", height);
				break;
			case 7:
				height = find_height(root_node);
				print_level_order(root_node, height);
				break;
			default:
				break;
		}
...
static int find_height(struct tree *root_node)
{
	...
	{
		left_height = find_height(root_node->left_child);
		right_height = find_height(root_node->right_child);
...
static int print_data(struct tree *root_node, int level)
{
	...
	{
		print_data(root_node->left_child, level - 1);
		print_data(root_node->right_child, level - 1);
	}
}
